<?php
// Path: admin/issue_schoolfee_receipt.php
// Now only one level up to reach the root
...
// --- HANDLE OFFICIAL RECEIPT UPLOAD BY ADMIN ---
if (isset($_POST['upload_official'])) {
    $user_id = $_POST['user_id'];
    
    // Path: up 1 level from admin/ folder to asset/
    $target_dir = "../asset/uploads/official_school_receipts/";
    if (!is_dir($target_dir)) mkdir($target_dir, 0777, true);

    if (!empty($_FILES['official_file']['name'])) {
        $ext = strtolower(pathinfo($_FILES['official_file']['name'], PATHINFO_EXTENSION));
        $filename = "official_school_fee_" . $user_id . "_" . time() . "." . $ext;
        
        if (move_uploaded_file($_FILES['official_file']['tmp_name'], $target_dir . $filename)) {
            $sql = "UPDATE fees_payments SET 
                    school_fee_official = '$filename', 
                    school_fee_status = 'Verified' 
                    WHERE user_id = '$user_id'";
            
            if (mysqli_query($conn, $sql)) {
                $msg = "<p style='color:green; padding:12px; background:#e8f5e9; border-left:5px solid #28a745; border-radius:4px; font-weight:bold; margin-bottom:20px;'>✔ Official Receipt issued successfully!</p>";
            }
        }
    }
}

// Fetch all students who have uploaded their Remita proof
$query = mysqli_query($conn, "SELECT login_table.id, login_table.fullname, login_table.email, 
    fees_payments.school_fee_remita, fees_payments.school_fee_status, fees_payments.school_fee_official 
    FROM login_table 
    INNER JOIN fees_payments ON login_table.id = fees_payments.user_id 
    WHERE fees_payments.school_fee_remita IS NOT NULL 
    ORDER BY fees_payments.date_uploaded DESC");
...
// Profile Image Path: up 1 level to reach asset/
...
<head>
    ...
    <title>Issue School Fees Receipt - Admin</title>
...
<body>
    ...
    <div class="body_area">
        <aside class="main_layout">
            <ul class="menu_list">
                ...
                <li class="menu_item">
                    ...
                    <ul class="submenu active" id="schoolfess">
                        <li><a href="#" style="color:#30e403;">Issue Sch. Fee Receipt</a></li>
                    </ul>
                </li>
                <li class="menu_item">
        ...
        <div class="body_div">
            <h2 style="color: #0e5001;">Verify School Fees Remita</h2>
            <p style="font-size: 13px; color: #777;">Review student Remita uploads and issue official university receipts.</p>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="../asset/uploads/school_fees_remita/<?= $row['school_fee_remita'] ?>" target="_blank" style="color:#007bff; font-weight:bold; text-decoration:none;">
                                👁️ VIEW REMITA
                            </a>
                        </td>
                        <td>
                            <span class="status_pill" style="<?= ($row['school_fee_status'] == 'Verified') ? 'background:#d4edda; color:#155724;' : 'background:#fff3cd; color:#856404;' ?>">
                                <?= strtoupper($row['school_fee_status']) ?>
                            </span>
                        </td>
                        <td>
                            <?php if (empty($row['school_fee_official'])): ?>
                                <form method="POST" enctype="multipart/form-data" style="display:flex; gap:5px;">
                                    ...
                                    <button type="submit" name="upload_official" style="background:#28a745; color:white; border:none; padding:5px 10px; border-radius:3px; cursor:pointer; font-size:10px; font-weight:bold;">UPLOAD OFFICIAL</button>
                                </form>
                            <?php else: ?>
