...
if (isset($_POST['upload_official_receipt'])) {
    ...
    if (!empty($_FILES['official_file']['name'])) {
        ...
        if (in_array($ext, $allowed)) {
            $new_name = "OFFICIAL_DEPT_" . $user_id . "_" . time() . "." . $ext;
            
            if (move_uploaded_file($_FILES['official_file']['tmp_name'], $target_dir . $new_name)) {
                // Update table with the official receipt name
                $sql = "UPDATE login_table SET original_dept_dues_receipt = '$new_name', dept_dues_status = 'Approved' WHERE id = '$user_id'";
                if (mysqli_query($conn, $sql)) {
                    $msg = "<p style='color:green; padding:10px; background:#d4edda; border-radius:5px;'>Official Receipt uploaded and student status updated!</p>";
                }
            } else {
...
// 3. Fetch all students who have uploaded tellers
$students = mysqli_query($conn, "SELECT id, fullname, email, dept_dues_teller, original_dept_dues_receipt FROM login_table WHERE dept_dues_teller IS NOT NULL ORDER BY id DESC");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Approve Departmental Dues - Admin</title>
...
<body>
    ...
    <div class="body_area">
            ...
            <ul class="menu_list">
                ...
                <li class="menu_item">
                    ...
                    <ul class="submenu active" id="department">
                        <li><a href="approve_dept.php" style="color:#30e403;">Issue Dept. Dues Receipt</a></li>
                    </ul>
                </li>
                <li class="menu_item">
        ...
        <div class="body_div">
            <h2 style="color: #0e5001;">Departmental Dues Verification</h2>
            <p>View uploaded tellers and issue official departmental receipts to students.</p>
            <?= $msg ?>
            ...
            <table class="admin-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if(!empty($row['original_dept_dues_receipt'])): ?>
                        ...
                        <td>
                            <a href="../asset/uploads/dept_payments/<?= $row['dept_dues_teller'] ?>" target="_blank" class="btn-view">View Teller</a>
                        </td>
                        <td>
                            <form action="" method="POST" enctype="multipart/form-data" class="upload-box">
                                ...
                                <button type="submit" name="upload_official_receipt" style="background:#0e5001; color:white; border:none; padding:5px 10px; cursor:pointer; font-size:10px; border-radius:3px;">Upload Receipt</button>
                            </form>
                            <?php if(!empty($row['original_dept_dues_receipt'])): ?>
                                <div style="margin-top:5px; font-size:10px; color:green;">File: <?= $row['original_dept_dues_receipt'] ?></div>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endwhile; ?>
