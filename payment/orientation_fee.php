<?php
// Path: payment/orientation_fee.php
...
// 1. Fetch User and Payment Data (using orientation specific columns)
$query = mysqli_query($conn, "SELECT login_table.id, login_table.fullname, login_table.profile_pic, 
    fees_payments.orientation_teller, fees_payments.orientation_status, 
    fees_payments.orientation_official_receipt 
    FROM login_table 
...
// 2. Handle Orientation Teller Upload
if (isset($_POST['upload_orientation_teller'])) {
    $target_dir = "../asset/uploads/orientation_tellers/";
    if (!is_dir($target_dir)) mkdir($target_dir, 0777, true);

    if (!empty($_FILES['ori_file']['name'])) {
        $ext = strtolower(pathinfo($_FILES['ori_file']['name'], PATHINFO_EXTENSION));
        $filename = "ori_teller_" . $user_id . "_" . time() . "." . $ext;
        
        if (move_uploaded_file($_FILES['ori_file']['tmp_name'], $target_dir . $filename)) {
            // Check if record exists in fees_payments
            $check = mysqli_query($conn, "SELECT * FROM fees_payments WHERE user_id = '$user_id'");
            
            if (mysqli_num_rows($check) > 0) {
                $sql = "UPDATE fees_payments SET orientation_teller = '$filename', orientation_status = 'Pending' WHERE user_id = '$user_id'";
            } else {
                $sql = "INSERT INTO fees_payments (user_id, orientation_teller, orientation_status) VALUES ('$user_id', '$filename', 'Pending')";
            }
            
            if (mysqli_query($conn, $sql)) {
                header("Location: orientation_fee.php?success=1");
                exit();
            }
        }
    }
...
if(isset($_GET['success'])) {
    $msg = "<p style='color:green; padding:12px; background:#e8f5e9; border-left:5px solid #28a745; border-radius:4px; font-weight:bold; margin-bottom:20px;'>✔ Orientation Teller uploaded successfully! Admin will verify and upload your official receipt soon.</p>";
}
...
<head>
    ...
    <title>Orientation Fee - User Portal</title>
...
<body>
    ...
    <div class="body_area">
            ...
            <ul class="menu_list">
                ...
                <li class="menu_item">
                    ...
                    <ul class="submenu active" id="payment">
                        ...
                        <li><a href="#" style="color:#30e403;">Pay Orientation Fee</a></li>
        ...
        <div class="body_div">
            <h2 style="color: #0e5001;">Freshers Orientation Fee</h2>
            <p style="font-size: 13px; color: #777; margin-bottom: 20px;">Pay your orientation fee to the specified account and upload the teller for your official receipt.</p>
            ...
            <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                ...
                <div style="flex: 1; min-width: 300px; background: white; padding: 25px; border-radius: 8px; border-top: 4px solid #0e5001; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                    ...
                    <div style="background: #fdfae6; padding: 15px; border-radius: 5px; font-size: 13px; margin-bottom: 20px; border: 1px dashed #e6ae03;">
                        <p><strong>Bank:</strong> EBSU MicroFinance Bank</p>
                        <p><strong>Account Name:</strong> EBSU Orientation Fee</p>
                        <p><strong>Acct No:</strong> 2033445566</p>
                        <p><strong>Amount:</strong> ₦500</p>
                    </div>

                    <?php if (empty($user['orientation_teller'])): ?>
                        <form method="POST" enctype="multipart/form-data">
                            <label style="font-size: 11px; font-weight: bold; display: block; margin-bottom: 5px;">UPLOAD BANK TELLER (IMAGE/PDF):</label>
                            <input type="file" name="ori_file" required style="margin-bottom: 15px; font-size: 12px;">
                            <button type="submit" name="upload_orientation_teller" style="background:#0e5001; color:white; border:none; padding:12px; border-radius:4px; width:100%; cursor:pointer; font-weight:bold;">SUBMIT TELLER</button>
                        </form>
                    <?php else: ?>
                        <div style="text-align: center; padding: 10px; background: #e8f5e9; color: #2e7d32; border-radius: 5px; font-weight: bold; font-size: 12px;">
                            ✔ TELLER UPLOADED (<?= $user['orientation_status'] ?>)
                        </div>
                    <?php endif; ?>
                </div>

                <div style="flex: 1; min-width: 300px; background: white; padding: 25px; border-radius: 8px; border-top: 4px solid #0e5001; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                    <h4 style="margin-bottom: 15px;">Step 2: Official Receipt</h4>
                    
                    <?php if (!empty($user['orientation_official_receipt'])): ?>
                        <div style="text-align: center; background: #f0fdf4; border: 1px solid #bbf7d0; padding: 20px; border-radius: 8px; margin-bottom: 15px;">
                            <p style="color: #166534; font-size: 13px; margin-bottom: 15px;">Your orientation payment has been verified. Download your official receipt below.</p>
                            <a href="../asset/uploads/orientation_receipts/<?= $user['orientation_official_receipt'] ?>" download style="display: block; text-align: center; background: #28a745; color: white; padding: 12px; border-radius: 4px; text-decoration: none; font-weight: bold; font-size: 12px;">DOWNLOAD OFFICIAL RECEIPT</a>
                        </div>
                    <?php else: ?>
                        <div style="text-align: center; padding: 30px 10px; color: #999;">
                            <svg width="50" height="50" fill="#ddd" viewBox="0 0 16 16" style="margin-bottom:10px;">
                                <path d="M14 14V4.5L9.5 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2zM9.5 3A1.5 1.5 0 0 0 11 4.5h2V14a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1h5.5v2z"/>
                            </svg>
                            <p style="font-size: 12px;">Your Official Receipt will be available for download here once the Admin verifies your bank teller.</p>
                        </div>
                    <?php endif; ?>
                </div>
