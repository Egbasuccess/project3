...
$query = mysqli_query($conn, "SELECT login_table.id, login_table.fullname, login_table.profile_pic, 
    fees_payments.medical_teller, fees_payments.medical_status, 
    fees_payments.medical_tracking_id, fees_payments.medical_official_receipt 
    FROM login_table 
...
// 2. Handle Medical Teller Upload
if (isset($_POST['upload_medical_teller'])) {
    $target_dir = "../asset/uploads/medical_tellers/";
    if (!is_dir($target_dir)) mkdir($target_dir, 0777, true);

    if (!empty($_FILES['med_file']['name'])) {
        $ext = strtolower(pathinfo($_FILES['med_file']['name'], PATHINFO_EXTENSION));
        $filename = "med_teller_" . $user_id . "_" . time() . "." . $ext;
        
        if (move_uploaded_file($_FILES['med_file']['tmp_name'], $target_dir . $filename)) {
            // Check if record exists in fees_payments
            $check = mysqli_query($conn, "SELECT * FROM fees_payments WHERE user_id = '$user_id'");
            
            if (mysqli_num_rows($check) > 0) {
                $sql = "UPDATE fees_payments SET medical_teller = '$filename', medical_status = 'Pending' WHERE user_id = '$user_id'";
            } else {
                $sql = "INSERT INTO fees_payments (user_id, medical_teller, medical_status) VALUES ('$user_id', '$filename', 'Pending')";
            }
            
            if (mysqli_query($conn, $sql)) {
                header("Location: medical_fee.php?success=1");
                exit();
            }
        }
    }
...
if(isset($_GET['success'])) {
    $msg = "<p style='color:green; padding:12px; background:#e8f5e9; border-left:5px solid #28a745; border-radius:4px; font-weight:bold; margin-bottom:20px;'>✔ Medical Teller uploaded successfully! Admin will provide your Tracking ID soon.</p>";
}
...
<head>
    ...
    <title>Medical Fee - User Portal</title>
...
<body>
    ...
    <div class="body_area">
            ...
            <ul class="menu_list">
                ...
                <li class="menu_item">
                    ...
                    <ul class="submenu active" id="payment">
                        <li><a href="#" style="color:#30e403;">Pay Medical Fee</a></li>
        ...
        <div class="body_div">
            <h2 style="color: #0e5001;">Medical Fee & Registration</h2>
            <p style="font-size: 13px; color: #777; margin-bottom: 20px;">Pay ₦500 to the medical unit and upload your teller to get your Unique Tracking ID.</p>
            ...
            <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                ...
                <div style="flex: 1; min-width: 300px; background: white; padding: 25px; border-radius: 8px; border-top: 4px solid #0e5001; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                    ...
                    <div style="background: #f9f9f9; padding: 15px; border-radius: 5px; font-size: 13px; margin-bottom: 20px; border: 1px dashed #ccc;">
                        <p><strong>Bank:</strong> Zenith Bank</p>
                        <p><strong>Account:</strong> EBSU Medical Services</p>
                        <p><strong>Acct No:</strong> 1022334455</p>
                        <p><strong>Amount:</strong> ₦500</p>
                    </div>

                    <?php if (empty($user['medical_teller'])): ?>
                        <form method="POST" enctype="multipart/form-data">
                            <label style="font-size: 11px; font-weight: bold; display: block; margin-bottom: 5px;">UPLOAD BANK TELLER:</label>
                            <input type="file" name="med_file" required style="margin-bottom: 15px; font-size: 12px;">
                            <button type="submit" name="upload_medical_teller" style="background:#0e5001; color:white; border:none; padding:12px; border-radius:4px; width:100%; cursor:pointer; font-weight:bold;">SUBMIT TELLER</button>
                        </form>
                    <?php else: ?>
                ...
                <div style="flex: 1; min-width: 300px; background: white; padding: 25px; border-radius: 8px; border-top: 4px solid #0e5001; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                    <h4 style="margin-bottom: 15px;">Step 2: Medical Clearance</h4>
                    
                    <?php if (!empty($user['medical_tracking_id'])): ?>
                        <div style="text-align: center; background: #e3f2fd; border: 1px solid #2196f3; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                            <small style="color: #555; text-transform: uppercase; font-size: 10px; font-weight: bold;">Unique Tracking ID</small>
                            <h2 style="color: #1976d2; margin: 5px 0; letter-spacing: 2px;"><?= $user['medical_tracking_id'] ?></h2>
                        </div>
                        <a href="../asset/uploads/medical_receipts/<?= $user['medical_official_receipt'] ?>" download style="display: block; text-align: center; background: #28a745; color: white; padding: 10px; border-radius: 4px; text-decoration: none; font-weight: bold; font-size: 12px;">DOWNLOAD MEDICAL RECEIPT</a>
                    <?php else: ?>
                        <div style="text-align: center; padding: 30px 10px; color: #999;">
                            <p style="font-size: 12px;">Your Unique Medical Tracking ID and Official Receipt will appear here after Admin verification.</p>
                        </div>
                    <?php endif; ?>
                </div>
